...
class ByteStreamer:
    ...
    def cancel(self):
        """Cancel the cache cleaning task."""
    ...
    async def clean_cache(self) -> None:
        ...
        while True:
            try:
                await asyncio.wait([asyncio.sleep(self.clean_timer), self._stop_event.wait()], return_when=asyncio.FIRST_COMPLETED)
                if self._stop_event.is_set():
                    logger.debug("Cache cleaning task stopped")
                    break
                self.cached_file_ids.clear()
                logger.debug("Cleaned the cache")
            except asyncio.CancelledError:
                logger.debug("Cache cleaning task cancelled")
                break
